...
define(
    function (require) {
        ...
        var Album = Dialog.extend({
             ...
             * @property {string|HTMLElement[]} options.prevBtn 控件前一个的按钮的选择器
             * @property {string|HTMLElement[]} options.nextBtn 控件后一个的按钮的选择器
             * @property {string|HTMLElement[]} options.fullScreen 控件全屏点击的dom元素
             ...
             * @property {string|HTMLElement[]} options.content album的内容节点
             * @property {string|HTMLElement[]} options.baseEle 小图的某个父节点选择器
             * @property {string|HTMLElement[]} options.img 小图的选择器
             * @property {string} options.origin 图片的真实地址
             * @property {Object} options.contentStyle 修饰内部的样式
             */
            options: {
                trigger: 'click',
                nextBtn: '.next',
                ...
                imageTpl: require('./album/image.tpl'),
                // 选择器
                img: '.j-img',
                content: '.j-album-content',
                contentImg: '.j-album-img',
                id: {
                    value: '',
                    setter: function () {
                        return lib.guid();
            ...
            init: function () {
                ...
                this.baseEle = lib.get(this.get('baseEle'));
            ...
            _showImg: function (evt, callback) {
                ...
                // 获取已插入dom的图片
                var el = lib.get(this.get('contentImg', this.element));

                // 对图片的大小和外观进行修改
                if (isImageLoaded(el)) {
                    lib.setData(el, 'loaded', true);
                    this._setImgPosition(el, null, callback);
                }
                else {
                    lib.setData(el, 'loaded', false);
                    el.onload = function () {
                        lib.setData(el, 'loaded', true);
            ...
            _initImages: function () {
                var imgList = lib.query(this.get('img'),this.baseEle);

                util.each(imgList, function (el,i) {
            ...
            _setImgPosition: function (el) {
                ...
                // 图片还没全部载入
                if (!lib.getData(el, 'loaded')) {
                    return;
                }

                var size = getImageSize(el);

                var viewWidth = lib.width(this.contentBox) - this.get('contentStyle').marginRight;
